<script lang="ts">
  ...
  type SortKey = 'name' | 'author' | 'dateFrom';
  type SortDir = 'asc' | 'desc' | 'none';
  let sort = $state<{ key: SortKey | null; dir: SortDir }>({ key: null, dir: 'none' });
  ...
  const sorted = $derived.by(() => {
    if (!sort.key || sort.dir === 'none') return items;
  ...
  const ariaSort = (key: SortKey): 'ascending' | 'descending' | 'none' => {
    if (sort.key !== key || sort.dir === 'none') return 'none';
        ...
        <thead>
          <tr class="bg-gray-50 border-b border-gray-200">
            <th scope="col" class="text-left px-6 py-3" aria-sort={ariaSort('name')}>
              ...
              >
                Nazwa
                {#if sort.key === 'name' && sort.dir !== 'none'}
            ...
            <th
              scope="col"
              class="text-left px-4 py-3 w-px whitespace-nowrap"
            ...
            >
              ...
              >
                Autor
                {#if sort.key === 'author' && sort.dir !== 'none'}
                  {#if sort.dir === 'asc'}<IconArrowUp
                      aria-hidden="true"
                      width="12"
                      height="12"
            ...
            <th scope="col" class="px-4 py-3 w-32 text-center" aria-sort={ariaSort('dateFrom')}>
              <div class="flex justify-center">
                ...
                >
                  Data od
                  {#if sort.key === 'dateFrom' && sort.dir !== 'none'}
                    {#if sort.dir === 'asc'}<IconArrowUp
                        aria-hidden="true"
                        width="12"
                        height="12"
